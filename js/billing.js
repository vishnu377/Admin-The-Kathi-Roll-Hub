...
window.openQrScanner = function() {
  ...
  document.getElementById('qr-scan-status').textContent = 'Camera starting...';
  ...
  navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
    .then(stream => {
      ...
      video.play();
      document.getElementById('qr-scan-status').textContent = 'QR dhundh raha hoon...';
      _startQrDecode(video);
    })
    .catch(err => {
      document.getElementById('qr-scan-status').textContent = '❌ Camera permission do';
      console.warn('Camera error:', err);
    });
};

function _startQrDecode(video) {
    ...
    if (code && code.data) {
      ...
      if (raw.startsWith('KRH:')) {
        ...
        if (/^\d{10}$/.test(mobile)) {
          ...
          cbToast('QR scan hua! Profile load ho raha hai...');
          cbLookup();
        } else {
          document.getElementById('qr-scan-status').textContent = '❌ Invalid QR — dobara try karo';
        }
      } else {
        document.getElementById('qr-scan-status').textContent = '⚠️ Yeh Kathi Roll Hub QR nahi hai';
      }
    }
  }, 300);
}
