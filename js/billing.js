...
async function cbLoadOffers() {
  ...
  listEl.innerHTML = '<p style="font-size:11px;color:var(--txt3);margin-bottom:8px">Multiple offers select kar sakte ho — sabhi ek saath stack honge</p>'
    + offers.map((o, i) => `<span class="offer-chip" onclick="cbSelectOffer(${i})" data-idx="${i}">${o.label} ${o.pct > 0 ? '(' + o.pct + '%)' : (o.flat > 0 ? '(₹' + o.flat + ')' : '')}</span>`).join('');
  window._cbOffersCache = offers;

  // ── AUTO APPLY all eligible offers ───────────────
  cbAppliedOffers = [...offers];
  document.querySelectorAll('.offer-chip').forEach(c => c.classList.add('on'));
  cbRenderSummary();
}
...
function cbTotalDiscount(subtotal) {
  ...
  cbAppliedOffers.forEach(o => {
    if (o.flat > 0) disc += o.flat;
    else if (o.pct > 0) disc += Math.round(subtotal * (o.pct / 100));
  });
...
window.cbConfirm = async function() {
  ...
  for (const appliedOffer of cbAppliedOffers) {
    ...
    const offerShare = appliedOffer.flat > 0 ? appliedOffer.flat : Math.round(subtotal * (appliedOffer.pct / 100));
    ...
    try {
      ...
      if (rwDoc.exists()) {
        ...
        await updateDoc(rwRef, { usageCount: (rw.usageCount || 0) + 1, usedBy, savedAmount: (parseInt(rw.savedAmount) || 0) + offerShare, savedAmounts: amounts });
      }
  ...
  if (!saveOk) { alert('⚠️ Kuch save nahi hua — internet check karo aur dobara try karo.'); return; }
  cbLastBill = { name: cbUser.name, mobile: mob, amt: subtotal, final, disc, pay: cbPay, points: newPoints, pointsEarned: ptsAdd, visits: newVisits, billId, isMilestone: newVisits % goal === 0, rewardLabel: s.defaultVisitReward || 'FREE item', items: itemsSnapshot };
  ...
  itemsListEl.innerHTML = itemsSnapshot.map(i => `<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0;color:var(--txt2)"><span>${i.name}${i.variant ? ' (' + i.variant + ')' : ''} <span style="color:var(--txt3)">x${i.qty}</span></span><span style="font-weight:700;color:var(--txt)">₹${i.price * i.qty}</span></div>`).join('');
  document.getElementById('cs-subtotal').textContent = '₹' + subtotal;
  const discRowEl = document.getElementById('cs-disc-row');
  if (disc > 0) { discRowEl.style.display = 'flex'; document.getElementById('cs-disc-amt').textContent = '-₹' + disc; } else { discRowEl.style.display = 'none'; }
  ...
  if (amountDue > 0) { dueRowEl.style.display = 'flex'; document.getElementById('cs-due-amt').textContent = '₹' + amountDue; } else { dueRowEl.style.display = 'none'; }
};
...
// ── Edit Bill — NEW ──────────────────────────────────
window.cbEditBill = function() {
  document.querySelector('.pos-grid').style.display = 'grid';
  document.getElementById('cart-success').style.display = 'none';
  cbToast('✏️ Bill edit karo — phir confirm karo');
};

// ── Cancel Bill — NEW ─────────────────────────────────
window.cbCancelBill = async function() {
  if (!cbLastBill || !cbLastBill.billId) {
    cbToast('Cancel karne ke liye bill nahi mila');
    return;
  }
  const ok = window.confirm(
    'Bill cancel karo?\n\nCustomer: ' + cbLastBill.name +
    '\nAmount: ₹' + cbLastBill.final +
    '\n\nNote: Points manually reverse karna agar zaroorat ho'
  );
  if (!ok) return;
  try {
    await updateDoc(doc(db, 'bills', cbLastBill.billId), {
      status: 'cancelled',
      cancelledAt: new Date().toISOString()
    });
    cbToast('✅ Bill cancelled ho gaya!');
    setTimeout(() => cbNewBill(), 1500);
  } catch(e) {
    console.warn('Cancel failed:', e);
    cbToast('❌ Cancel nahi hua — internet check karo');
  }
};
...
window.openQrScanner = function() {
  ...
  .catch(err => {
    ...
    status.textContent = '❌ Camera permission do → Browser settings → Allow';
  });
};
