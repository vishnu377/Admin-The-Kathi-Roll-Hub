...
import { db, doc, updateDoc, getDoc, collection, query, where, getDocs } from '../shared/firebase-config.js';
...
window.bhCancelBill = async function(idx) {
  ...
  if (!confirm(`"${bill.name}" ka ₹${bill.final ?? bill.amt} ka bill cancel karna hai?\n\nPoints, visit, aur udhaar automatically reverse ho jayenge (agar applicable ho).`)) return;
  ...
  try {
    ...
    // ── 2. Fetch fresh customer data ───────────────────────
    const custRef = doc(db, 'users', bill.mobile);
    ...
    if (custSnap.exists()) {
      ...
      // ── Safety check: only reverse visit/points if this was
      //    the customer's MOST RECENT visit. Reversing an old
      //    bill's visit count would corrupt the streak data.
      const isLatestVisit = bill.visitNumber && cust.visits === bill.visitNumber;

      if (isLatestVisit) {
        updates.visits = Math.max(0, (cust.visits || 0) - 1);
        updates.points = Math.max(0, (cust.points || 0) - (parseInt(bill.pointsEarned) || 0));
      } else {
        // Older bill — only reverse points (safe), leave visit count alone
        updates.points = Math.max(0, (cust.points || 0) - (parseInt(bill.pointsEarned) || 0));
      }

      // ── Reverse "saved" (discount savings) ────────────────
      if (bill.discount > 0) {
        updates.saved = Math.max(0, (cust.saved || 0) - (parseFloat(bill.discount) || 0));
      }

      // ── Reverse udhaar (if this bill added to their debt) ──
      if (bill.amountDue > 0) {
        updates.totalDebt = Math.max(0, (parseFloat(cust.totalDebt) || 0) - (parseFloat(bill.amountDue) || 0));
      }
    ...
    // ── 3. Reverse coupon usage (if a reward was used on this bill) ──
    if (bill.offer === 'reward') {
      try {
        const rwSnap = await getDocs(
          query(collection(db, 'rewards'), where('usedBy', 'array-contains', bill.mobile))
        );
        rwSnap.forEach(async (rwDoc) => {
          const rw = rwDoc.data();
          const usedBy = (rw.usedBy || []).filter(m => m !== bill.mobile);
          const amounts = { ...(rw.savedAmounts || {}) };
          delete amounts[bill.mobile];
          await updateDoc(doc(db, 'rewards', rwDoc.id), {
            usageCount: Math.max(0, (rw.usageCount || 0) - 1),
            usedBy,
            savedAmounts: amounts
          });
        });
      } catch (e) { console.warn('[bhCancelBill] reward reversal failed:', e.message); }
    }

    bill.status = 'cancelled';
    const realIdx = allBills.findIndex(b => b.id === bill.id);
    if (realIdx !== -1) allBills[realIdx].status = 'cancelled';
    bhToast('✅ Bill cancel ho gaya, sab kuch reverse ho gaya');
    renderStats();
    renderTable();
  } catch (e) {
